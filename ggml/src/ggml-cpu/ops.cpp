...
static void ggml_compute_forward_add_rms_norm_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    GGML_ASSERT(ggml_are_same_shape(src0, dst) && ggml_are_same_shape(src1, dst));
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                const float * s0 = (float *) ((char *) src0->data + i01*nb01 + i02*nb02 + i03*nb03);
                const float * s1 = (float *) ((char *) src1->data + i01*nb11 + i02*nb12 + i03*nb13);

                ggml_float sum = 0.0;
                for (int64_t i00 = 0; i00 < ne00; i00++) {
                    float v = s0[i00] + s1[i00];
                    sum += (ggml_float)(v * v);
                }
...
void ggml_compute_forward_add_rms_norm(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_add_rms_norm_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
