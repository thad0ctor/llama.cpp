...
// Blackwell HBM3 features require CUDA 11.8+ and compute capability 12.0+
...
__global__ void blackwell_hbm3_memcpy_kernel(
    ...
    using VecType = typename cuda::std::conditional<
        sizeof(T) == sizeof(float), float4,
        typename cuda::std::conditional<sizeof(T) == sizeof(half), half2, T>::type
    >::type;
...
bool validate_hbm3_optimizations(int device_id) {
    ...
    const bool has_high_bandwidth = device.hbm_bandwidth > 1000ULL * 1024 * 1024 * 1024; // > 1TB/s
