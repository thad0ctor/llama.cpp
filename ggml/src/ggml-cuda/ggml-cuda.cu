...
#include "ggml-cuda/mul_mat_tma.cuh"
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const int cc                 = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    // All sm_86+ GPUs have fast FP16, so always allow F16 batched cuBLAS
    bool use_batched_cublas_f16  = src0->type == GGML_TYPE_F16;
    bool use_batched_cublas_bf16 = src0->type == GGML_TYPE_BF16 && bf16_mma_hardware_available(cc);
    bool use_batched_cublas_f32  = src0->type == GGML_TYPE_F32;

    // TMA path for large F16/BF16 matrices on Blackwell (sm_120)
    const bool use_tma = !split && ggml_cuda_should_use_tma(src0, src1, cc);

    if (!split && use_tma) {
        ggml_cuda_mul_mat_tma(ctx, src0, src1, dst);
...
static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    // Disable CUDA graphs in presence of env var, use-case which is changing too rapidly,
